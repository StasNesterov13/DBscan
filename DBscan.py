...
def clicked():
    global borehole
    borehole = Borehole()
    borehole.Creature(txt1.get(), txt2.get(), int(txt3.get()), var.get())
    lbl1 = Label(window, text=borehole.name, width=15, height=1, font="Arial 14")
...
if __name__ == '__main__':
    ...
    btn11 = Button(window, text="График дебита воды", command=graphics_water, height=2, width=30)
    ...
    btn15 = Button(window, text="График обводненности", command=graphics_water_cut, height=2, width=30)
    btn15.grid(column=0, row=11)
    ...
    var = IntVar()
    btn13 = Radiobutton(text="Рприем, атм", variable=var, value=14)
    btn13.grid(column=1, row=6)
    btn14 = Radiobutton(text="Рзаб замер", variable=var, value=16)
    btn14.grid(column=1, row=7)
